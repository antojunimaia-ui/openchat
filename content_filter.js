...
class ContentFilter {
    constructor() {
        ...
        // ========== CONTEÚDO BLOQUEADO - Crimes e Filias Ilegais ==========
        // GUIDELINE 1.7: Bloqueio absoluto de crimes envolvendo filias (pedofilia, zoofilia, necrofilia, etc)
        this.blockedContentPatterns = [
            // ===== PEDOFILIA - INGLÊS =====
            /\b(pedo|pedoph|paedo|paedoph|pedophilia|paedophilia)\b/i,
            /\b(cp|child\s*porn|kiddie\s*porn|underage\s+porn)\b/i,
            /\b(loli|lolita|lolicon|shota|shotacon)\b/i,
            /\b(hebephilia|ephebophilia|hebephile|ephebophile)\b/i,
            /\b(jailbait|barely\s+legal)\b/i,
            /\b(minor|child|kid|toddler|baby|infant)\b.*\b(sex|porn|fuck|suck|dick|cock|pussy|vagina)\b/i,

            // ===== PEDOFILIA - PORTUGUÊS =====
            /\b(pedofilia|pedófilo|pedófila)\b/i,
            /\b(pornografia\s+infantil|porn\s+infantil|conteúdo\s+infantil)\b/i,
            /\b(hebefilia|efebofilia)\b/i,
            /\b(criança|menor|infantil|bebê|recém-nascido)\b.*\b(sexo|porn|fuder|transar|chupar|pau|buceta)\b/i,

            // ===== ZOOFILIA - INGLÊS =====
            /\b(zoophilia|zoophile|bestiality|agrizoophobia)\b/i,
            /\b(animal\s+sex|beast\s+sex|sex\s+with\s+animals|sex\s+with\s+dogs|sex\s+with\s+horses)\b/i,
            /\b(dogpill|knotting)\b/i,

            // ===== ZOOFILIA - PORTUGUÊS =====
            /\b(zoofilia|zoófilo|bestialidade)\b/i,
            /\b(sexo\s+com\s+animais|sexo\s+com\s+cachorro|sexo\s+com\s+cavalo)\b/i,
            /\b(transar\s+com\s+bicho|transar\s+com\s+animal)\b/i,

            // ===== NECROFILIA - INGLÊS =====
            /\b(necrophilia|necrophile|necrophilism)\b/i,
            /\b(corpse\s+sex|dead\s+body\s+sex|sex\s+with\s+dead)\b/i,

            // ===== NECROFILIA - PORTUGUÊS =====
            /\b(necrofilia|necrófilo)\b/i,
            /\b(sexo\s+com\s+cadáver|sexo\s+com\s+morto|transar\s+com\s+morto)\b/i,

            // ===== OUTRAS FILIAS CRIMINOSAS/ILEGAIS =====
            /\b(biastophilia|rape\s+fantasy)\b/i, // Estupro
            /\b(somnophilia|sleeping\s+sex)\b/i, // Sexo com pessoas dormindo/inconscientes

            // ===== EVASÕES GENÉRICAS PARA FILIAS =====
            /\b(p\s*e\s*d\s*o|z\s*o\s*o|n\s*e\s*c\s*r\s*o)\b/i,
            /\b(p3d0|z00|n3cr0)\b/i,
        ];
